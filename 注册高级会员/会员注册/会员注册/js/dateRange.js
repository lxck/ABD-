...
*/
    ...
    var defaults = {
		...
		aRecent365Days : 'aRecent365Days', //最近365天
                startDate : '', // 开始日期
                endDate : '', // 结束日期
                startCompareDate : '', // 对比开始日期
                endCompareDate : '', // 对比结束日期
	        minValidDate : '0', //最小可用时间，控制日期选择器的可选力度
	...
	this.periodObj[__method.mOpts.aRecent365Days] = 365;
    // 记录初始默认时间
    this.startDefDate = '';
    // 随机ID后缀
    ...
    $('#' + this.submitBtn).bind('click', function() {
        ...
        //处理日期变化产生的页面数值计算
        serviceDateChange($('#' + __method.mOpts.startDateId).val(),$('#' + __method.mOpts.endDateId).val());
        return false;
    });
    // 日期选择关闭按钮的 click 事件
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    today = new Date(yToday,mToday,dToday);
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
			...
			|| ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate * 1000)
